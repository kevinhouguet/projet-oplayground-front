...
function AutocompleteInput() {
  ...
  useEffect(() => {
    if (searchTerm !== "") {
      axios
        ...
        .get(
          `https://geo.api.gouv.fr/communes?nom=${searchTerm}&fields=code,nom,codesPostaux&limit=5`
        )
        .then((response) => {
  ...
  function handleSubmit(event) {
    ...
    axios
      .get(
        `https://oplaygroundapi.herokuapp.com/api/terrains?commune="${searchTerm}"&codepostal=${selectedCodePostal}`
      )
      .then((response) => {
